import { Resolver, Arg, Mutation, Authorized, Query, Ctx } from "type-graphql";
...
export class LoginResolver {
  ...
  @Authorized()
  @Query(() => [EventToken])
  async otherEventTokens(@Ctx() { auth }: Context): Promise<EventToken[]> {
    if (auth.isAdmin)
      ...
      return this.prisma.mentor
        .findMany({
          where: {
            OR: [
              { email: { equals: mentor.email, mode: "insensitive" } },
      ...
      return this.prisma.partner
            ...
            OR: [
              {
                event: {
                  students: {
                    some: { partnerCode: { equals: auth.partnerCode } },
                  },
                },
              },
        ...
        .then((partners) =>
          partners.map((partner) => ({
            event: partner.event,
            token: signTokenPartner(partner),
          })),
        );
    } else return [];
  }
