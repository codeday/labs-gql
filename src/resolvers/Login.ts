...
export class LoginResolver {
  ...
  async otherEventTokens(
    ...
    if (auth.isAdmin) return this.prisma.event.findMany().then(events => events.map(event => ({ event, token: signTokenAdmin(event) })));
      ...
      const student = await this.prisma.student.findUniqueOrThrow({ where: idOrUsernameOrAuthToUniqueWhere(auth) });
      ...
      const mentor = await this.prisma.mentor.findUniqueOrThrow({ where: idOrUsernameOrAuthToUniqueWhere(auth) });
