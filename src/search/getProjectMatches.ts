...
import { Student, Match, Project, Tag } from "../types";
...
const DEBUG = makeDebug("search:getProjectMatches");
...
function buildTagsScore(tags: Tag[]): ScoreFunction[] {
  ...
  return tags.map(({ id, type }) =>
    esb
      ...
      .filter(
        esb.matchQuery(
          type === TagType.INTEREST ? Entry.interestTags : Entry.stackTags,
          id,
        ),
      )
      .weight(
        (type === TagType.INTEREST
          ? SCORE_WEIGHTS.TAG_INTEREST_MATCH
          : SCORE_WEIGHTS.TAG_TECHNOLOGY_MATCH) /
...
function isStudentUnderrepresented(student: Student): boolean {
  ...
  if (
    !["he/him", "he/him/his"].includes(
      <string | undefined>profile?.pronouns || "he/him",
    )
  )
    return true;
  if (
    !["white"].includes((<string>profile?.ethnicity || "white").toLowerCase())
  )
    return true;
  return false;
}
...
async function getTimezone(student: Student): Promise<number> {
  ...
  const profile = <
    { location?: { postal?: string; country?: string } } | undefined
  >student?.profile;
...
function buildAffinityQuery(
  partner?: Partner | null,
): esb.MatchQuery | esb.BoolQuery {
  if (!partner) return esb.matchQuery(Entry.affinePartnerId, ELASTIC_NULL);
  if (partner.onlyAffine)
    return esb.matchQuery(Entry.affinePartnerId, partner.id);
  return esb
    .boolQuery()
    .should([
...
async function buildQueryFor(
  ...
  const filteredTags =
    partner?.forbidTags && partner.forbidTags.length > 0
  ...
  const partnerForbidTags =
    partner?.forbidTags && partner.forbidTags.length > 0
      ? esb
          .boolQuery()
          .mustNot(
            partner.forbidTags.map(({ id }) => esb.termQuery("tags", id)),
          )
      : undefined;

  const queryStudentRequiresLength = esb
    .rangeQuery(Entry.maxWeeks)
    .gte(student.weeks);
  ...
  const scorePopularity = esb
    .decayScoreFunction("gauss", Entry.studentsSelected)
    .origin(0)
  ...
  return esb
    ...
    .query(
      esb
        .boolQuery()
        .must(
          <(RangeQuery | TermQuery | BoolQuery)[]>(
            [
              partnerForceTags,
              partnerForbidTags,
...
export async function getProjectMatches(
  ...
  const query = esb
    .requestBodySearch()
    .query(await buildQueryFor(student, tags))
    .toJSON();
  ...
  const hits = <
    {
      _id: string;
      _score: number;
      _explanation: Record<string, unknown> | undefined;
    }[]
  >body.hits.hits;
