...
export async function generateFileForIndividuals(
  fileType: FileType,
  event: Event,
  ...
  for (const individual of individuals) {
    try {
      ...
      DEBUG(`Error generating ${fileType.id} for ${isStudent(individual) ? 'student' : 'mentor'} ${individual.id}:`);
      DEBUG(ex);
    }
  }
}
...
async function generateFileForProjects(
  ...
  for (const project of projects) {
    try {
      ...
      DEBUG(`Error generating ${fileType.id} for project ${project.id}:`);
      DEBUG(ex);
    }
  }
}
