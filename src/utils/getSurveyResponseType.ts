...
export function getSurveyResponseType(
  ...
  if (
    (response.mentorId && response.authorMentorId && response.mentorId === response.authorMentorId)
    || (response.studentId && response.authorStudentId && response.studentId === response.authorStudentId))
    {
