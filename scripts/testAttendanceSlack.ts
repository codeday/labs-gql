...
import { buildWeeklyAttendanceAlertMessage, AttendanceIssue } from '../src/automation/tasks/sendAttendanceAlerts';
import { getSlackClientForEvent } from '../src/slack';
import { isAttendanceTracked } from '../src/utils';
...
async function collectAttendanceIssuesFromEvent(
    ...
    return { students, untrackedProjectCount };
}
...
async function postTestMessage(
    ...
    const message = {
        ...
        text: buildWeeklyAttendanceAlertMessage(eventName, students, untrackedProjectCount),
    };
...
async function main() {
    ...
    try {
        ...
        let untrackedProjectCount: number;
        ...
        if (useRealData) {
            ...
            untrackedProjectCount = issues.untrackedProjectCount;
            eventName = sourceEvent.name;

            console.log(`Using event data: ${sourceEvent.name} (${sourceEvent.id})`);
            console.log(`Found ${students.length} flagged students (${untrackedProjectCount} untracked project(s)) in local DB\n`);

            if (students.length === 0) {
                console.error('❌ Seeded test data did not produce any student attendance issues.');
                process.exit(1);
            }
        } else {
        ...
        if (isDryRun) {
            await postTestMessage(null, channelName, eventName, students, untrackedProjectCount);
            return;
        }
        ...
        await postTestMessage(slack, channelName, eventName, students, untrackedProjectCount);
    } finally {
